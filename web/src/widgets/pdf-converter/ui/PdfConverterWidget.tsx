...
export default function PdfConverterWidget() {
  ...
  const handleConvertClick = async () => {
    ...
    if (blob) {
      const zipName = `converted-${file.name.replace(/\.pdf$/i, '')}.zip`;
      showSidebar(new File([blob], zipName, { type: 'application/zip' }), {
        title: '변환 완료',
        lines: [`${targetFormat.toUpperCase()} 이미지 ZIP`, `결과 ${formatSize(blob.size)}`],
      });
      setFile(null);
    }
  };
    ...
    <div className="flex w-full min-h-screen">
      <aside className="fixed top-16 left-0 bottom-0 w-80 z-40 flex flex-col border-r border-gray-200 bg-white dark:bg-gray-800 dark:border-gray-700 shadow-sm" aria-label="기능 컨트롤">
        <div className="p-4 border-b border-gray-100 dark:border-gray-700">
          <h1 className="text-lg font-bold text-gray-800 dark:text-white">PDF 변환</h1>
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">PDF → PNG, JPG</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-col gap-1.5">
              <button
                ...
                className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors text-left ${targetFormat === 'png' ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}
              >
                PNG로 변환
              </button>
              <button
                onClick={() => setTargetFormat('jpeg')}
                className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors text-left ${targetFormat === 'jpeg' ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'}`}
              >
                JPG로 변환
              </button>
            </div>
          ...
          <DownloadBtn
            text={isConverting ? '변환 중...' : `${targetFormat.toUpperCase()}로 변환하기`}
      ...
      <main className="flex-1 flex flex-col items-center justify-center min-h-0 p-6 overflow-auto bg-gray-50 dark:bg-gray-900/50 ml-80 relative">
        ...
        <div className="flex flex-col items-center w-full max-w-4xl">
          <div className="flex items-center justify-center min-h-[300px] w-full">
            <img
              src={previews[file.name] || PLACEHOLDER}
              alt="미리보기"
              className="max-h-[70vh] max-w-full object-contain border rounded-lg shadow-lg bg-white"
            />
          </div>
